' Toy+ - Console version, floating point, includes math functions, for FreeBasic
...
sub run_pgm(interactive as boolean, filename as string)
    if not interactive then open filename for input as #1
    init_g_sym_tab()
    init_l_sym_tab()
    init_pending_list()
    init_code()
...
sub interpret()
    ...
    dim as double stack(MAX_STACK), argf, argf2   'primary datatype
